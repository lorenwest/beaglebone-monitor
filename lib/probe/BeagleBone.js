...
var BeagleBone = Probe.extend({
  ...
  initialize: function(params, options) {
    ...
    // The pin map by ID keys on BeagleBone pin ID
    // key=>BeagleBone ID 'P9_22'  value=>Pin object
    t.pinMapById = {};
    ...
    var pins = Config.BeagleBone && Config.BeagleBone ? Config.BeagleBone.pins : null;
    if (pins) {
      ...
      // Configure the pins
      logger.info('init.localPins', 'Configuring local pins', pins);
  ...
  * @param [pins.n.id] {String} The beaglebone ID of the pin ('P9_22')
  ...
  definePins_control: function(pins, callback) {
    ...
    pins.forEach(function(pin) {
      ...
      if (pin.id) {
        ...
        // Is this a known pin?
        if (!bonePins[pin.id]) {
          return callback({code: 'UNKNOWN_PIN', msg: 'Pin id not found: ' + pin.id, pin: pin});
        }
        pin.bonePin = bonePins[pin.id];

        // Is this pin id already defined?
        if (t.pinMapById[pin.id]) {
          return callback({code: 'PIN_IN_USE', msg: 'Pin is already in use: ' + pin.id, pin: pin});
        }
      }
  ...
  * @param pins.n.id {String} The beaglebone pin ID (P9_22)
